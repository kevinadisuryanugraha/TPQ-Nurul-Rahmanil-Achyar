...
@section('page_title', 'CMS Landing Page - Ubah Testimoni')
...
<div class="max-w-2xl bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="p-6 border-b border-gray-100 bg-white flex items-center justify-between">
        <div>
            <h3 class="text-lg font-bold text-gray-800">Form Ubah Testimoni</h3>
            <p class="text-xs text-gray-500 mt-1">Lakukan pembaruan terhadap konten ulasan dari wali santri.</p>
        </div>
    ...
    <form action="{{ route('admin.landing.testimoni.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            @if($testimonial->foto)
                <img src="{{ $testimonial->foto }}" alt="{{ $testimonial->nama }}" class="w-20 h-20 rounded-full object-cover border border-gray-200 shadow-sm">
            @else
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" required value="{{ old('nama', $testimonial->nama) }}" placeholder="Contoh: Ibu Fatimah"
            ...
            <div>
                ...
                <input type="text" name="role" id="role" required value="{{ old('role', $testimonial->role) }}" placeholder="Contoh: Orang Tua dari Ahmad (Iqra 4)"
                       class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs">
                @error('role')
                    <p class="text-[10px] text-rose-500 mt-1">{{ $message }}</p>
                @enderror
        ...
        <div>
            ...
            <textarea name="isi" id="isi" rows="4" required placeholder="Tuliskan ulasan wali santri secara lengkap (Maksimal 500 karakter)..."
                      class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs">{{ old('isi', $testimonial->isi) }}</textarea>
            @error('isi')
                <p class="text-[10px] text-rose-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="number" name="urutan" id="urutan" value="{{ old('urutan', $testimonial->urutan) }}" min="1"
                       class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs">
                @error('urutan')
                    <p class="text-[10px] text-rose-500 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="is_active" id="is_active" required
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs bg-white">
                    <option value="1" {{ old('is_active', $testimonial->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Aktif (Tampilkan)</option>
                    <option value="0" {{ old('is_active', $testimonial->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>Nonaktif (Sembunyikan)</option>
                </select>
                @error('is_active')
                    <p class="text-[10px] text-rose-500 mt-1">{{ $message }}</p>
                @enderror
