...
@section('page_title', 'CMS Landing Page - Tambah Foto')
...
<div class="max-w-2xl bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="p-6 border-b border-gray-100 bg-white flex items-center justify-between">
        <div>
            <h3 class="text-lg font-bold text-gray-800">Form Unggah Foto</h3>
            <p class="text-xs text-gray-500 mt-1">Tambahkan foto dokumentasi baru beserta kategori dan urutan penampilannya.</p>
        </div>
    ...
    <form action="{{ route('admin.landing.galeri.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
        @csrf

        <!-- Gambar -->
        <div>
            <label for="gambar" class="block text-xs font-bold text-gray-700 mb-2">Pilih File Foto <span class="text-rose-500">*</span></label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="text" name="judul" id="judul" value="{{ old('judul') }}" placeholder="Contoh: Belajar Quran Level Iqra 3"
            ...
            <div>
                ...
                <input type="text" name="kategori" id="kategori" value="{{ old('kategori') }}" placeholder="Contoh: Kegiatan Kelas, Tahfidz, Ibadah"
                       class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs">
                @error('kategori')
                    <p class="text-[10px] text-rose-500 mt-1">{{ $message }}</p>
                @enderror
