...
@section('title', 'Edit Panduan Praktik')
@section('page_title', 'Edit Panduan Praktik')

@section('content')
<div class="max-w-3xl mx-auto">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('admin.konten.panduan.show', $panduan->id) }}" class="text-sm font-semibold text-emerald-700 hover:text-emerald-800 flex items-center space-x-1">
            <i class="fa-solid fa-arrow-left"></i>
            <span>Kembali ke Detail Panduan</span>
        </a>
    </div>

    <!-- Main Card Form -->
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.konten.panduan.update', $panduan->id) }}" method="POST" enctype="multipart/form-data" id="panduan-form">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="judul" required value="{{ old('judul', $panduan->judul) }}" placeholder="Contoh: Panduan Praktik Wudhu Sesuai Sunnah"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="jenis_praktik" required value="{{ old('jenis_praktik', $panduan->jenis_praktik) }}" placeholder="Contoh: Wudhu, Shalat, Tayammum" list="jenis_list"
                    ...
                    <div>
                        ...
                        <select name="level_target_id" 
                            ...
                            @foreach($levels as $lvl)
                                <option value="{{ $lvl->id }}" {{ old('level_target_id', $panduan->level_target_id) == $lvl->id ? 'selected' : '' }}>{{ $lvl->nama }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" required rows="3" placeholder="Jelaskan mengenai panduan praktik ini..."
                        class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 @error('deskripsi') border-rose-500 @enderror">{{ old('deskripsi', $panduan->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="border border-gray-300 rounded-xl p-4 bg-gray-50 flex items-center justify-between">
                        ...
                        <div id="image-preview-container" class="{{ $panduan->cover_image ? '' : 'hidden' }}">
                            <img id="image-preview" src="{{ $panduan->cover_image ?? '#' }}" alt="Preview" class="w-16 h-16 object-cover rounded-lg border border-gray-200">
                        </div>
                    </div>
                    @error('cover_image')
                        <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-6 mt-2">
                        <label class="flex items-center">
                            <input type="radio" name="status" value="published" required {{ old('status', $panduan->status) == 'published' ? 'checked' : '' }}
                        ...
                        <label class="flex items-center">
                            <input type="radio" name="status" value="draft" required {{ old('status', $panduan->status) == 'draft' ? 'checked' : '' }}
                                class="h-4 w-4 border-gray-300 text-emerald-600 focus:ring-emerald-500">
