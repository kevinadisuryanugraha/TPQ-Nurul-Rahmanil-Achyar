...
Route names follow: `superadmin.*`, `admin.*`, `murid.*`, `admin.landing.*`, `daftar.*`

## Architecture

- **Admin Panel**: Livewire 4 + Alpine.js — `@livewireStyles`/`@livewireScripts` in `layouts/admin.blade.php`
...
- Existing tests cover: auth, admin CRUD (murid, absensi, penilaian, konten, pengumuman, laporan), PSB flow, public routes
...
- **Font Awesome** loaded via CDN (`cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css`) in both admin and murid layouts
- **PWA**: custom service worker at `public/sw.js` (no npm package, no Workbox) — SW registration is inline in `layouts/murid.blade.php`, not in app.js
